...
export default function MoodlyApp() {
  ...
  const [profile, setProfile] = useState({ age:"", gender:"", customGender:"", country:"Nepal", languages:["English"] as string[], terms:false });
  ...
  if (view === "auth") return <Auth email={email} setEmail={setEmail} magicSent={magicSent} onMagic={() => setMagicSent(true)} onBack={() => setView("welcome")} onSuccess={() => setView("onboarding")}/>;
  if (view === "onboarding") return <Onboarding profile={profile} setProfile={setProfile} onDone={() => {
  ...
  return (
    <main className={`app-shell ${view === "chat" ? "chat-bg":""}`}>
      ...
      {view === "mode" && <section className="panel compact-panel">
        ...
        <button className="primary wide" onClick={() => { setQueueSeconds(0); setView("queue"); }}>Find someone <span>→</span></button>
      ...
      {view === "survey" && <section className="panel compact-panel survey-panel">
        ...
        <button className="primary wide" onClick={() => { setToast("Thanks — your response was saved."); setView("home"); }}>Submit response</button>
      ...
      {view === "settings" && <Settings profile={profile} setProfile={setProfile} onBack={() => setView(prior)}/>}
...
function Settings({profile,setProfile,onBack}:{profile:any,setProfile:(p:any)=>void,onBack:()=>void}){ return <section className="settings-view"><button className="back" onClick={onBack}>←</button><span className="overline">ACCOUNT SETTINGS</span><h1>Your private profile</h1><p>These details are never visible to conversation partners.</p><label>Country<select value={profile.country} onChange={e=>setProfile({...profile,country:e.target.value})}>{countries.map(c=><option key={c}>{c}</option>)}</select></label><div className="settings-card"><span className="avatar large-avatar">SL</span><div><b>Your anonymous name changes every 24 hours</b><p>Today's name: <strong>Quiet Sparrow</strong></p></div></div><button className="primary" onClick={onBack}>Save changes</button></section>; }
