...
export async function sendMagicLinkEmail(
  ...
  const html = [
    ...
    `<p>Use the secure button below to sign in as ${safeRecipient}.</p>`,
    `<p style=\"margin:28px 0\"><a href=\"${safeLink}\" style=\"display:inline-block;background:#214f3b;color:#fff;text-decoration:none;padding:13px 22px;border-radius:999px;font-weight:700\">Open Moodly</a></p>`,
    `<p style=\"font-size:13px;color:#5d7067\">This link expires in 15 minutes and can be used once. If you did not request it, you can ignore this email.</p>`,
    `<p style=\"font-size:12px;color:#7b8b84;word-break:break-all\">${safeLink}</p>`,
    "</div></body></html>",
  ].join("\r\n");
  const headersAndBody = [
    `From: Moodly <${username}>`,
    `To: ${recipient}`,
    "Subject: Your secure Moodly sign-in link",
